...
//x2[n]=X+a*x1[n-delta_T],量化a,采用CSD编码
//0010_0101_0001
//00b0 b000 b010
//00b0_1000_100b
	assign X2=X+
...
//sig_delay#(DW,1)comb1x1_delay1(clk,rst_n,x1,x1_d1);
    ...
    assign x1_d1=x1d1;
//x1[n]=x2[n]+g*x1[n-1],其中g=0.077,量化后g=315.3920=315=1_0011_1011=csd1_0100_0-10-1
//1000_b0b0_0b00
	assign X1=X2-
	{{10{x1_d1[DW-1]}},x1_d1,2'b0}-
	{{7{x1_d1[DW-1]}},x1_d1,5'b0}-
...
//-----------------------对中间信号x1延时--------------------------------
    ...
    always @(posedge clk or negedge rst_n)begin
        if(~rst_n)RdEnx=1'b0;
        else if(Wnumx>=350)RdEnx=1'b1;
        else RdEnx=1'b0;
        end
	fifo_sc_top comb1_FIFOx(
